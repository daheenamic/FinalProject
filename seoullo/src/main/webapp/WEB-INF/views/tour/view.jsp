...
<!-- 				버튼 DIV -->
				<div>
					...
					<c:if test="${!empty login && login.gradeNo == 1}">
						<button id="bookBtn" type="button" class="btn btn-default btn-xs" >예약하기</button>
						<button id="cartBtn" type="button" class="btn btn-default btn-xs" >장바구니</button>
					</c:if>
	...
	<script type="text/javascript">
		$(function(){
			...
			$("#cartBtn").click(function(){
				let tourPriceInput = $(this).parents("#rightDiv").find("#tourPriceInput").val();
				let formData = $('form[name=cartForm]').serialize(); // form안에 있는 데이터 가져오기
				if(tourPriceInput === "0") {
					alert("예약하실 인원을 선택 해 주세요");
					return false;
				}
